...
*** Mac OS X
  ...
  The download is only available for 64bit based OS X installs. Riak
  can be downloaded in this format here:
  64bit:  http://downloads.basho.com/riak_0.9.0.osx.tar.gz
...
*** Network Ports
    ...
    + 4369: Erlang Port Mapper Daemon's (EPMD) port, for connecting
      Riak nodes.  Which network interface on which this port is
      exposed can be configured by specifying a value for
      =inet_dist_user_interface= in the =kernel= section of Riak's
      =app.config= (see example below).

    + 5001-6024: Dynamically allocated ports by EPMD.  This range can
      be modified by specifying values for =inet_dist_listen_min= and
      =inet_dist_listen_max= in Riak's =app.config= (see example
      below).

    + 8098: Default HTTP interface port.  This port can be modified by
      specifying a value for =riak_web_port= in the =riak= section of
      Riak's =app.config= (see example below).  The network interface
      on which this port is exposed can be configured by specifying a
      value for =riak_web_ip= in the =riak= section of Riak's
      =app.config=.
...
#+END_SRC
   ...
   The value of =riak_web_ip= is a string representation of an IP
   address.  To specify that Webmachine should listen on all
