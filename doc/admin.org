...
** The =riak-admin= script
   ...
   + wait-for-service :: check that a Riak node is fully up (useful for scripting)

               This command will check that a given riak service is fully initialed
   ...
   + transfers :: list any pending transfers of partitions from one node to another

               Provides a list of nodes with pending partition transfers (i.e. any secondary vnodes)
               and lists any owned vnodes that are *not* running.
