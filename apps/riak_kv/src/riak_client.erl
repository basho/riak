...
send_event(EventName, EventDetail) ->
    rpc:call(Node,riak_core_eventer,notify,
...
%% See {@link riak_core_eventer:add_handler/4.} for more information.
add_event_handler(Pid, Desc, MatchHead, MatchGuard) ->
    rpc:call(Node, riak_core_eventer, add_handler, [Pid, Desc, MatchHead, MatchGuard]).

%% @doc
%% Remove an event handler added by {@link add_event_handler/4}, if it exists.
%% See {@link riak_core_eventer:remove_handler/3.} for more information.
