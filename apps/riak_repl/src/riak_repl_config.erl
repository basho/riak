...
-export([set_ring/1, add_local_listener/3]).
-record(state, {ring}).
...
add_local_listener(NodeName, ListenIP, Port) ->
...
handle_add_local_listener(Node, ListenIP, Port, State=#state{ring=Ring}) ->
    ...
    NewListeners = 
        case proplists:get_value(Node, LocalListeners) of
            ...
            Listeners ->
                [{Node, {ListenIP, Port}}|proplists:delete(Node, Listeners)]
        end,
    NewRing = case NewListeners =:= LocalListeners of
        true -> 
            Ring;
        false -> riak_core_ring_manager:set_my_ring(
