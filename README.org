...
** JDK required to build Riak Enterprise

   Riak Enterprise exposes monitoring data via JMX. To compile JMX,
   JDK must be installed before building Riak Enterprise. 

** Building Riak

   Assuming you have a working Erlang (R14B04 or later) installation,
